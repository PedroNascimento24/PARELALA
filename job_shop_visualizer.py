...
def read_job_shop_file(filename):
    ...
    # A primeira linha pode conter múltiplos inteiros, usar apenas o primeiro como makespan
    makespan = int(lines[0].strip().split()[0])
            ...
            for pair in pairs:
                if ',' in pair:
                    try:
                        ...
                        continue  # Ignora pares malformados
            if start_times:  # Só adiciona se houver dados válidos
...
def create_job_shop_gantt(jobs_file, mapping_file, output_image):
    ...
    if not jobs_start_times:
        print(f"ERRO: Nenhum dado de operações encontrado em {jobs_file}. Verifique o arquivo de saída.")
        return
    # Sempre usar o mapeamento padrão para garantir compatibilidade com o output
    ...
    base_colors = ['#e6194b', '#3cb44b', '#0082c8', '#f58231', '#911eb4', '#46f0f0']
    num_jobs = len(jobs_start_times)
    colors = base_colors.copy()
    if num_jobs > len(colors):
        for _ in range(num_jobs - len(colors)):
            hex_color = "#{:06x}".format(random.randint(0, 0xFFFFFF))
            colors.append(hex_color)
    # Mapear cada job a uma cor fixa
    job_color_map = {job_idx: colors[job_idx % len(colors)] for job_idx in range(num_jobs)}
    ...
    for machine_idx, operations in sorted(machine_operations.items()):
        ...
        for op in operations:
            ...
            rect = patches.Rectangle(
                ...
                linewidth=1,
                edgecolor='black',
                facecolor=job_color_map[job_idx],
                alpha=0.85
            )
            ax.add_patch(rect)
    ...
    handles = [patches.Patch(color=job_color_map[j], label=f'Job {j}') 
...
def main():
    ...
    parser.add_argument('--output_file', type=str, required=True, help='Output file for the Job Shop solution or output image')
    ...
    # If output_file ends with .png, use it directly as the image path
    if output_file.lower().endswith('.png'):
        output_image = output_file
    else:
        output_image = f".\\viz\\gantt_{output_file.replace('.jss', '.png').replace('.txt', '.png')}"
