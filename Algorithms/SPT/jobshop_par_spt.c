// jobshop_par_spt.c
// Parallel job shop scheduler using parallel Shortest Processing Time (SPT) heuristic
...
void get_result_path_par(char* path, const char* algorithm, const char* size_category, const char* basename, int nth) {
    sprintf(path, "../../Result/%s/%s/%s_spt_par_%d.txt", algorithm, size_category, basename, nth);
}
...
int parallel_spt_schedule(ParallelShop *shop, int nth, int should_log) {
    ...
    while (count < total && iter < maxit) {
        ...
        #pragma omp parallel num_threads(nth) reduction(+:did)
        {
            ...
            {
                // Find the operation with shortest processing time among available operations for this thread's jobs
...
void dump_logs_par(ParallelShop *shop, int nth, const char *basename) {
    ...
    get_log_path_par(tfile, algorithm, size_category, basename, "timing_parspt", nth);
    get_log_path_par(sfile, algorithm, size_category, basename, "sequence_parspt", nth);
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < reps; i++) {
        ...
        parallel_spt_schedule(&shop, nthr, (i == reps-1)); // Only log on final iteration
    ...
    system(cmd);
#endif
    sprintf(sumfile, "../../Logs/%s/%s/%s_exec_parspt.txt", algorithm, size_category, base);
    FILE *fsum = fopen(sumfile, "a");
    if (fsum) {
        fprintf(fsum, "Input: %s, Threads: %d, ParSPT, AvgTime: %.9f s\n", base, nthr, avg);
        fclose(fsum);
    }
    return 0;
}
