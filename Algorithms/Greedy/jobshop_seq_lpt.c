// jobshop_seq_lpt.c
// Sequential job shop scheduler using Longest Processing Time (LPT) heuristic
...
void get_result_path(char* path, const char* algorithm, const char* size_category, const char* basename) {
    sprintf(path, "../../Result/%s/%s/%s_lpt_seq.txt", algorithm, size_category, basename);
}
...
// LPT Scheduling: Always select operation with longest processing time
void lpt_schedule(Shop *shop) {
    int done[JMAX] = {0};
    int nextst[JMAX] = {0};
    ...
    while (count < total) {
        ...
        // Find the operation with longest processing time among available operations
        for (int j = 0; j < shop->njobs; j++) {
            if (done[j] < shop->nops) {
                int o = done[j];
...
void dump_logs_seq(Shop *shop, const char *basename) {
    ...
    get_log_path(tfile, algorithm, size_category, basename, "timing_seqlpt");
    get_log_path(sfile, algorithm, size_category, basename, "sequence_seqlpt");
...
int main(int argc, char *argv[]) {
    ...
    system(cmd);
#endif
    sprintf(sumfile, "../../Logs/%s/%s/%s_exec_seqlpt.txt", algorithm, size_category, base);
    FILE *fsum = fopen(sumfile, "a");
    if (fsum) {
        fprintf(fsum, "Input: %s, SeqLPT, AvgTime: %.9f s\n", base, avg);
        fclose(fsum);
    }
    return 0;
}
