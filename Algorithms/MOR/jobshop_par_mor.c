// jobshop_par_mor.c
// Parallel job shop scheduler using parallel Most Operations Remaining (MOR) heuristic
...
void get_result_path_par(char* path, const char* algorithm, const char* size_category, const char* basename, int nth) {
    sprintf(path, "../../Result/%s/%s/%s_mor_par_%d.txt", algorithm, size_category, basename, nth);
}
...
int parallel_mor_schedule(ParallelShop *shop, int nth, int should_log) {
    ...
    while (count < total && iter < maxit) {
        ...
        #pragma omp parallel num_threads(nth) reduction(+:did)
        {
            ...
            {
                // Find the job with most operations remaining among this thread's jobs
                int bestj = -1, besto = -1, mostops = -1;
                
                for (int j = 0; j < shop->njobs; j++) {
                    if (job_thread[j] == tid && done[j] < shop->nops) {
                        int remaining = shop->nops - done[j];
                        if (remaining > mostops) {
                            mostops = remaining;
                            bestj = j;
                            besto = done[j];
                        }
                ...
                if (bestj != -1) {
                    ...
                    int st = find_slot_par(shop, m, l, nextst[bestj]);
                    
                    // Only schedule if not already scheduled
                    if (shop->plan[bestj][besto].stime == -1) {
                        ...
                        if (should_log) {
                            ...
                            if (idx < LOGMAX) {
                                shop->tlogs[tid][idx].jid = bestj;
                                shop->tlogs[tid][idx].oid = besto;
...
void dump_logs_par(ParallelShop *shop, int nth, const char *basename) {
    ...
    get_log_path_par(tfile, algorithm, size_category, basename, "timing_parmor", nth);
    get_log_path_par(sfile, algorithm, size_category, basename, "sequence_parmor", nth);
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < reps; i++) {
        ...
        parallel_mor_schedule(&shop, nthr, (i == reps-1)); // Only log on final iteration
    ...
    system(cmd);
#endif
    sprintf(sumfile, "../../Logs/%s/%s/%s_exec_parmor.txt", algorithm, size_category, base);
    FILE *fsum = fopen(sumfile, "a");
    if (fsum) {
        fprintf(fsum, "Input: %s, Threads: %d, ParMOR, AvgTime: %.9f s\n", base, nthr, avg);
        fclose(fsum);
    }
    return 0;
}
