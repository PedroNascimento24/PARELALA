// jobshop_seq_mor.c
// Sequential job shop scheduler using Most Operations Remaining (MOR) heuristic
...
void get_result_path(char* path, const char* algorithm, const char* size_category, const char* basename) {
    sprintf(path, "../../Result/%s/%s/%s_mor_seq.txt", algorithm, size_category, basename);
}
...
// MOR Scheduling: Prioritize jobs with most operations remaining
void mor_schedule(Shop *shop) {
    int done[JMAX] = {0};
    int nextst[JMAX] = {0};
    ...
    while (count < total) {
        ...
        // Find the job with most operations remaining
        for (int j = 0; j < shop->njobs; j++) {
            if (done[j] < shop->nops) {
...
void dump_logs_seq(Shop *shop, const char *basename) {
    ...
    get_log_path(tfile, algorithm, size_category, basename, "timing_seqmor");
    get_log_path(sfile, algorithm, size_category, basename, "sequence_seqmor");
...
int main(int argc, char *argv[]) {
    ...
    system(cmd);
#endif
    sprintf(sumfile, "../../Logs/%s/%s/%s_exec_seqmor.txt", algorithm, size_category, base);
    FILE *fsum = fopen(sumfile, "a");
    if (fsum) {
        fprintf(fsum, "Input: %s, SeqMOR, AvgTime: %.9f s\n", base, avg);
        fclose(fsum);
    }
    return 0;
}
